...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from AndroPlus Github (http://github.com/AndroPlusRom)." % device

repositories = []

page = 1
while not depsonly:
    result = json.loads(urllib2.urlopen("https://api.github.com/users/AndroPlusRom/repos?page=%d" % page).read())
    if len(result) == 0:
        break
    for res in result:
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            ...
            if existing_project.attrib['revision'] == repository['branch']:
                print 'Team-AndroPlus/%s already exists' % (repo_name)
            else:
        ...
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": repo_name, "revision": "ap5.0" })
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                print 'AndroPlusRom/%s already exists' % (repo_name)
            else:
                print 'updating branch for AndroPlus/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: AndroPlusRom/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "AndroPlusRom/%s" % repo_name, "revision": "ap5.0" })
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/ap.dependencies'
    syncable_repos = []
...
if depsonly:
    ...
    for repository in repositories:
        ...
        if repo_name.startswith("device_") and repo_name.endswith("_" + device):
            ...
            add_to_manifest([{'repository':repo_name,'target_path':repo_path,'branch':'ap5.0'}])
...
print "Repository for %s not found in the AndroPlusRom Github repository list. If this is in error, you may need to manually add it to .repo/local_manifests/local_manifest.xml" % device
